...
from view import update_ssl, clean, extract_sds, extract_sec
...
class App(Window):
    ...
    def create_widget(self):
        ...
        Button(cmd_panel, text="CoA", command=self.generate_coa, state="disabled").pack(side="left")
    ...
    def generate_coa(self):
        ...
        for row in selected:
            if row.sds is not None:
                ...
                img_b = pre_cut(sds_img, cut_bg=True)
                lines = gel_crop(img_b)

                img = np.array(cv2.imread(sds_img))
                non_reduced = img[:, lines[lane-1]:lines[lane]]
                marker = img[:, lines[7]:lines[8]]
                reduced = img[:, lines[7+lane]:lines[8+lane]]
                folder = askdirectory()
                cv2.imwrite(f"{folder}/sds.png", np.hstack([non_reduced, marker, reduced]))
            if row.sec is not None:
